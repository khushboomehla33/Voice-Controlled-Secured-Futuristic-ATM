...
def main():
    ...
    mixer.init()
    mixer.music.load('audio\\withdraw.mp3')
    mixer.music.play()
    def listen():
        global result
    ...
    new_thread = threading.Thread(
            ...
            kwargs={})
    new_thread.start()
    root.after(2000, listen_for_result)
    ...
    heading=Label(full,font=('Kacst Screen',40,'bold'),text="Enter the amount you want to withdraw",anchor='w',fg='black',bg='lightskyblue')
    heading.place(x=120,y=15)

    uid=Label(full,font=('arial',32,'bold'),text="AMOUNT",anchor='w',fg='black',bg='lightskyblue')
    uid.place(x=235,y=120)
    ...
    def sub():
        code= utils.removebalance(int(text_Input.get()))
        if(code==1):
            msg="Withdraw successful. Your new balance is "+str(utils.getbalance())
            root.destroy()
            ts.main(msg)
        elif(code==0):
            msg="Could not withdraw the amount"
            root.destroy()
            tc.main(msg)
        elif(code==-1):
            msg="You do not have sufficient balance to withdraw"
            root.destroy()
            tc.main(msg)
        else:
            msg="error"
