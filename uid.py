...
def main():
    ...
    mixer.init()
    mixer.music.load('audio\\uid.mp3')
    mixer.music.play()
    ...
    heading=Label(full,font=('Kacst Screen',48,'bold'),text="Enter or Speak UID provided By Bank",anchor='w',fg='black',bg='lightskyblue')
    heading.place(x=25,y=15)

    uid=Label(full,font=('arial',40,'bold'),text="UID",anchor='w',fg='Black',bg='lightskyblue')
    uid.place(x=350,y=120)
    def sub():
        #utils.set_text(txtDisplay,"21321")
        uid= text_Input.get()
        root.destroy()
        root.quit()
        gc.collect()
    ...
    #txtDisplay=Entry(calculator,font=('arial',17,'bold'),bd=30,insertwidth=4,bg='white',justify='center')
